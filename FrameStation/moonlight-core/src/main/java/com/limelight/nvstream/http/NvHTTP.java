...
public class NvHTTP {
    ...
    // XR-REMOVED: BuildConfig.DEBUG replaced with false (no Android BuildConfig)
    ...
    private void initializeHttpState(final LimelightCryptoProvider cryptoProvider) {
        ...
        trustManager = new X509TrustManager() {
            ...
            public void checkServerTrusted(X509Certificate[] certs, String authType) throws CertificateException {
                try {
                    ...
                    else if (NvHTTP.this.serverCert == null) {
                        // XR-MODIFIED: Accept self-signed certs when no server cert is pinned yet.
                        // This is needed for the initial pairing connection (trust-on-first-use).
                        // After pairing, the server cert will be pinned.
                        LimeLog.info("Accepting unpinned server certificate for initial connection");
                    }
                    else {
                        // The cert chain doesn't look like a self-signed cert
                        throw e;
                    }
                }
            }
        ...
        HostnameVerifier hv = new HostnameVerifier() {
            public boolean verify(String hostname, SSLSession session) {
                ...
                // XR-MODIFIED: When no server cert is pinned (initial connection or paired without
                // cert storage), accept self-signed certs from GameStream servers.
                // These certs typically have CN=Sunshine/Apollo but no SAN for the IP address.
                if (NvHTTP.this.serverCert == null) {
                    try {
                        Certificate[] certs = session.getPeerCertificates();
                        if (certs.length == 1) {
                            LimeLog.info("Accepting hostname for unpinned server cert: " + hostname);
                            return true;
                        }
                    } catch (SSLPeerUnverifiedException e) {
                        // fall through
    ...
    public String getServerInfo(boolean likelyOnline) throws IOException, XmlPullParserException {
        ...
        if (serverCert != null) {
            ...
            // No pinned cert â€” use HTTP directly. Apollo/Sunshine return serverinfo
            // over HTTP without requiring client cert authentication.
            return openHttpConnectionToString(client, baseUrlHttp, "serverinfo");
        }
    }
